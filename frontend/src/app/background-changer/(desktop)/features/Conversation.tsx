...
import useBackgroundChanger from '../../provider/useBackgroundChanger';
...
const Conversation = memo((props: ConversationProps) => {
  ...
  const { generationIdRef, currentHistory, previewImageSrc, setPreviewImageSrc } = useBackgroundChanger();
  ...
  const handleCreateImage = async function () {
    ...
    try {
      ...
      // let negativePrompt = props.backgroundNegativePrompt;
      ...
      const promises = Array.from({ length: props.fetchCount }).map(async (_, index: number) => {
        const response = await AxiosProvider.post(`/api/image-generation/bg_change_b?clientId=${clientId}`, {
          ...bgChangeRequest,
