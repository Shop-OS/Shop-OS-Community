...
const PromptInput = ({
  ...
  const { generationIdRef, currentHistory } = useProductBG();
        ...
        {showCopyButton && (
          <Popover
            content={<div style={{
              ...
              {currentHistory?.generations?.[0].prompt}
              <div style={{
                display: "flex",
                justifyContent: "end",
                ...
                onClick={() => {
                  navigator.clipboard.writeText(currentHistory?.generations?.[0].prompt);
                }}
              >
                <Copy size={18} />
              </div>
            ...
            onOpenChange={(val) => {
              if (currentHistory?.generations?.[0]?.prompt)
                setOpen(val);
            }}
          >
            <Button
