...
export default function MotionMaskModal({
  ...
  const canvasRef1: CanvasRef = useRef(null);
  ...
  const [scale2, setScale] = useState(1);

  const zoomIn = () => {
    const newScale = scale * 1.1;
    setScale(newScale);
    // const ctx1: any = canvasRef1?.current?.getContext('2d');
    // const ctx2: any = canvasRef2?.current?.getContext('2d');
    // ctx1.scale(newScale, newScale);
    // ctx2.scale(newScale, newScale);
  ...
  const redrawCanvas = (newScale: any) => {
    const ctx1: any = canvasRef1?.current?.getContext('2d');
    const ctx2: any = canvasRef2?.current?.getContext('2d');

    // Get the data for the entire canvas
    const data1 = ctx1.getImageData(0, 0, canvasRef1?.current?.width, canvasRef1?.current?.height);
    const data2 = ctx2.getImageData(0, 0, canvasRef2?.current?.width, canvasRef2?.current?.height);

    // Create temporary canvases
    const tempCanvas1 = document.createElement('canvas');
    const tempCanvas2 = document.createElement('canvas');
    const tempCtx1: any = tempCanvas1.getContext('2d');
    const tempCtx2: any = tempCanvas2.getContext('2d');

    // Draw the image data onto the temporary canvases
    tempCtx1.putImageData(data1, 0, 0);
    tempCtx2.putImageData(data2, 0, 0);

    // Clear the canvas
    ctx1.clearRect(0, 0, canvasRef1?.current?.width, canvasRef1?.current?.height);
    ...
    // Draw the image data from the temporary canvases onto the scaled canvases
    // Draw the image data from the temporary canvases onto the scaled canvases
    ctx1.drawImage(
      tempCanvas1,
      0,
  ...
  // const redrawCanvas = (newScale: any) => {
  //   let canvas1: any = canvasRef1?.current;
  //   const ctx1: any = canvas1?.getContext('2d');
  //   // Change the height of the canvas

  //   let canvas2: any = canvasRef2?.current;
  ...
  //   tempCanvas1.height = canvas1.height * newScale;
  //   tempCanvas1.width = canvas1.width * newScale;
  //   tempCanvas2.height = canvas2.height * newScale;
  //   tempCanvas2.width = canvas2.width * newScale;

  //   // Draw the image data onto the temporary canvases
  //   tempCtx1.putImageData(data1, 0, 0);
  //   tempCtx2.putImageData(data2, 0, 0);

  //   canvas1.height = tempCanvas1.height;
  //   canvas1.width = tempCanvas1.width;

  //   canvas2.height = tempCanvas2.height;
  //   canvas2.width = tempCanvas2.width;

  //   // Clear the canvas
  //   ctx1.drawImage(tempCanvas1, 0, 0, tempCanvas1.width, tempCanvas1.height, 0, 0, (canvasRef1?.current?.width ?? 0) * newScale, (canvasRef1?.current?.height ?? 0) * newScale);
  //   ctx2.drawImage(tempCanvas2, 0, 0, tempCanvas2.width, tempCanvas2.height, 0, 0, (canvasRef2?.current?.width ?? 0) * newScale, (canvasRef2?.current?.height ?? 0) * newScale);
  // };
  ...
  let scale = 1;
  let imageData1: any = null;
  let imageData2: any = null;

  const handleWheel = (event: any) => {
    const canvas1: any = canvasRef1.current;
    const canvas2: any = canvasRef2.current;
    const context1: any = canvas1.getContext('2d');
    const context2: any = canvas2.getContext('2d');

    if (!imageData1) {
      imageData1 = context1.getImageData(0, 0, canvas1.width, canvas1.height);
    }
    if (!imageData2) {
      imageData2 = context2.getImageData(0, 0, canvas2.width, canvas2.height);
    }
    // Adjust the scale based on the wheel event
    if (event.deltaY < 0) {
      scale *= 1.1;
    } else {
      scale /= 1.1;
    ...
    context1.drawImage(tempCanvas1, 0, 0, canvas1.width * scale, canvas1.height * scale);
    context2.drawImage(tempCanvas2, 0, 0, canvas2.width * scale, canvas2.height * scale);
  };
  ...
  const handleMouseMove2 = (event: any) => {
    ...
    const xDiff = event.clientX - startCoords[0];
    const yDiff = event.clientY - startCoords[1];
    setStartCoords([event.clientX, event.clientY]);

    context1.translate(xDiff / scale, yDiff / scale);
    context2.translate(xDiff / scale, yDiff / scale);
  ...
  useEffect(() => {
    ...
    return () => {
      canvas1.removeEventListener('wheel', handleWheel);
      canvas2.removeEventListener('wheel', handleWheel);
    };
  }, []);
  ...
  return (
    ...
    >
      ...
      <div style={{ maxHeight: '400px', overflow: 'visible' }}>
        <div
          ref={containerRef}
          style={{
            display: 'flex',
      ...
      <div style={{ display: 'flex', alignItems: 'center', marginTop: 20 }}>
        ...
        <button
          ...
          style={{
            ...
            backgroundColor: productMaskDescription ? 'rgba(58,58,58,1)' : 'rgba(31,31,31,1)',
            fontSize: '14px',
            fontWeight: '500',
