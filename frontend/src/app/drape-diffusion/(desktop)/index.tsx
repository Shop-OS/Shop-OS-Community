...
import { OutfitProvider } from '../provider/OutfitProvider';
...
const DesktopPage = () => {
  ...
  const [ootdType, setOotdType] = useState('upper_body');
  ...
  useEffect(() => {
    let showTour = Cookies.get('hom_tour_drape_diffusion') == 'true' ? true : false;

    if (!showTour) {
      Cookies.set('hom_tour_drape_diffusion', 'true', { expires: 365 });
      setShowTour(true);
      setTourType('Drape Diffusion');
    }
  }, []);
