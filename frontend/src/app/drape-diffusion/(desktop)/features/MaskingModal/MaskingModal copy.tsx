...
export default function MaskingModal({
  idx,
  imageSrc,
  previewImage,
  toggleModal,
  ...
  const { maskImage, pinkMaskImage, setPinkMaskImage } = useOutfit();
  ...
  useEffect(() => {
    ...
    img1.onload = function () {
      ...
      img2.src = pinkMaskImage ? pinkMaskImage : maskImage; // replace with your mask image path
    };
  ...
  const createImageData = (canvasRef: CanvasRef) => {
    ...
    setPinkMaskImage(canvas?.toDataURL('image/png'));
  ...
  const saveMaskImageCopy = () => {
    ...
    img.onload = function () {
      if (tempCanvas) {
        ...
        console.log("first", whiteMaskDataUrl)
