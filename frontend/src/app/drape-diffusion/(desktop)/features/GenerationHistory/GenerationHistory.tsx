...
import useOutfit from '@/app/drape-diffusion/provider/useOutfit';



function GenerationHistory() {
    const { generationHistory, setGenerationHistory, currentHistory, setCurrentHistory, previewImageSrc, setPreviewImageSrc } = useOutfit();
    ...
    const getHistory = async () => {
        try {
            ...
            setCurrentHistory({});
            const response = await AxiosProvider.get('api/generations?target=backgroundChangeO');
    ...
    return (
        ...
        >
            ...
            >
                ...
                (!generationHistory || generationHistory?.length == 0) ? "No history found" :
                    generationHistory.map((item: any, index: number) => (
                        ...
                        >
                            <img
                                height={40}
                                src={item.inputClothImageURL ?? "https://dfstudio-d420.kxcdn.com/wordpress/wp-content/uploads/2019/06/digital_camera_photo-1080x675.jpg"}
                                alt="image"
                                style={{
                                    borderRadius: '4px',
                                }}
